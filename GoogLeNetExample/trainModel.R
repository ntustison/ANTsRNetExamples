...
# googLeNetModel <- createGoogLeNetModel2D( c( dim( trainingImageArrays[[1]] ), 1 ),
#   numberOfClassificationLabels = numberOfLabels )
googLeNetModel <- createGoogLeNetModel2D( dim( trainingImageArrays[[1]] ),
  numberOfClassificationLabels = numberOfLabels )

if( numberOfLabels == 2 )   
  {
  googLeNetModel %>% compile( loss = 'binary_crossentropy',
...
track <- googLeNetModel %>% fit( X_train, Y_train, 
                 epochs = 20, batch_size = 32, verbose = 1, shuffle = TRUE,
                #  callbacks = list( 
                #    callback_model_checkpoint( paste0( baseDirectory, "googLeNetWeights.h5" ),
...
save_model_weights_hdf5( 
  googLeNetModel, filepath = paste0( baseDirectory, 'googLeNetWeights.h5' ) )
save_model_hdf5( 
  googLeNetModel, filepath = paste0( baseDirectory, 'googLeNetModel.h5' ), overwrite = TRUE )

## Plot the model fitting

# epochs <- 1:length( track$metrics$loss )

# googLeNetModelDataFrame <- data.frame( Epoch = rep( epochs, 2 ), 
...
# googLeNetModelLossPlot <- ggplot( data = googLeNetModelDataFrame, aes( x = Epoch, y = Loss, colour = Type ) ) +
#                  geom_point( shape = 1, size = 0.5 ) +
#                  geom_line( size = 0.3 ) +
#                  ggtitle( "Loss" )
                

# googLeNetModelAccuracyPlot <- ggplot( data = googLeNetModelDataFrame, aes( x = Epoch, y = Accuracy, colour = Type ) ) +
#                  geom_point( shape = 1, size = 0.5 ) +
#                  geom_line( size = 0.3 ) +
#                  ggtitle( "Accuracy")

# ggsave( paste0( baseDirectory, "googLeNetModelLossPlot.pdf" ), plot = googLeNetModelLossPlot, width = 5, height = 2, units = 'in' )
# ggsave( paste0( baseDirectory, "googLeNetModelAccuracyPlot.pdf" ), plot = googLeNetModelAccuracyPlot, width = 5, height = 2, units = 'in' )
