...
source( paste0( modelDirectory, 'createDenseNetModel.R' ) )
...
# denseNetModel <- createDenseNetModel2D( c( dim( trainingImageArrays[[1]] ), 1 ),
#   numberOfClassificationLabels = numberOfLabels )
denseNetModel <- createDenseNetModel2D( dim( trainingImageArrays[[1]] ),
  numberOfClassificationLabels = numberOfLabels )

if( numberOfLabels == 2 )   
  {
  denseNetModel %>% compile( loss = 'binary_crossentropy',
...
track <- denseNetModel %>% fit( X_train, Y_train, 
                 epochs = 20, batch_size = 32, verbose = 1, shuffle = TRUE,
                 callbacks = list( 
                   callback_model_checkpoint( paste0( baseDirectory, "denseNetWeights.h5" ),
...
save_model_weights_hdf5( 
  denseNetModel, filepath = paste0( baseDirectory, 'denseNetWeights.h5' ) )
save_model_hdf5( 
  denseNetModel, filepath = paste0( baseDirectory, 'denseNetModel.h5' ), overwrite = TRUE )

## Plot the model fitting

# epochs <- 1:length( track$metrics$loss )

# denseNetModelDataFrame <- data.frame( Epoch = rep( epochs, 2 ), 
...
# denseNetModelLossPlot <- ggplot( data = denseNetModelDataFrame, aes( x = Epoch, y = Loss, colour = Type ) ) +
#                  geom_point( shape = 1, size = 0.5 ) +
#                  geom_line( size = 0.3 ) +
#                  ggtitle( "Loss" )
                

# denseNetModelAccuracyPlot <- ggplot( data = denseNetModelDataFrame, aes( x = Epoch, y = Accuracy, colour = Type ) ) +
#                  geom_point( shape = 1, size = 0.5 ) +
#                  geom_line( size = 0.3 ) +
#                  ggtitle( "Accuracy")

# ggsave( paste0( baseDirectory, "denseNetModelLossPlot.pdf" ), plot = denseNetModelLossPlot, width = 5, height = 2, units = 'in' )
# ggsave( paste0( baseDirectory, "denseNetModelAccuracyPlot.pdf" ), plot = denseNetModelAccuracyPlot, width = 5, height = 2, units = 'in' )
