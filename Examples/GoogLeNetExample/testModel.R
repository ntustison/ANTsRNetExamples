...
baseDirectory <- './'
dataDirectory <- paste0( baseDirectory, '../GoogLeNetExample/Images/' )
...
# googLeNetModelTest <- createGoogLeNetModel2D( c( dim( testingImageArrays[[1]] ), 1 ),
#   numberOfClassificationLabels = numberOfLabels, style = 19 )
googLeNetModelTest <- createGoogLeNetModel2D( dim( testingImageArrays[[1]] ),
  numberOfClassificationLabels = numberOfLabels )
if( numberOfLabels == 2 )   
  {
  googLeNetModelTest %>% compile( loss = 'binary_crossentropy',
    optimizer = optimizer_adam( lr = 0.0001 ),  
    metrics = c( 'binary_crossentropy', 'accuracy' ) )
  } else {
  googLeNetModelTest %>% compile( loss = 'categorical_crossentropy',
    optimizer = optimizer_adam( lr = 0.0001 ),  
    metrics = c( 'categorical_crossentropy', 'accuracy' ) )
  }

load_model_weights_hdf5( googLeNetModelTest, filepath = paste0( baseDirectory, 'googLeNetWeights.h5' ) )

testingMetrics <- googLeNetModelTest %>% evaluate( X_test, Y_test )
predictedData <- googLeNetModelTest %>% predict( X_test, verbose = 1 )
