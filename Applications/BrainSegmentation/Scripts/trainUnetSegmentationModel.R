...
for( i in 1:length( brainImageFiles ) )
  {
  trainingImageFiles[[count]] <- c( brainImageFiles[i], brainMaskFiles[i] )
  trainingSegmentationFiles[[count]] <- brainSegmentationFiles[i]

  subjectId <- basename( brainSegmentationFiles[i] )
  subjectId <- sub( "BrainSegmentation.nii.gz", '', subjectId )
  ...
  if( !file.exists( fwdtransforms[1] ) || !file.exists( fwdtransforms[2] ) ||
    ...
    {
    stop( paste( "Transform", xfrmPrefix, "file does not exist.\n" ) )
    }
